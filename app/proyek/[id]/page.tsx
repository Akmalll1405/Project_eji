...
export default function DetailProyekPage() {
  ...
  const handleUpdateProyek = async () => {
    if (isLocked && (session?.user as any)?.role !== 'ADMIN') {
      alert('Proyek terkunci. Ajukan izin edit ke Admin.')
      return
    }
    ...
    if (editProyekForm.tanggalSelesai && new Date(editProyekForm.tanggalSelesai) < new Date(editProyekForm.tanggalMulai)) {
  ...
  const handleUploadDokumen = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    while (attempt < 3) {
      ...
      try {
        ...
        if (attempt >= 3) alert(`Gagal upload setelah 3x percobaan.\n${error.message}`)
      ...
      <div className="mb-4 px-4 py-3 rounded-xl"
        ...
        <div className="flex items-start justify-between gap-3">
          ...
          <button onClick={() => setShowRequestEditModal(true)}
            className="px-3 rounded-lg text-xs font-medium whitespace-nowrap flex-shrink-0"
  ...
  return (
    <div style={{ minHeight: '100dvh', background: '#030712' }}>
      ...
      >
        ...
        {/* Nav Tab */}
        <div
          className="flex gap-1 mb-6 pb-1"
          style={{
          ...
          <div>
              ...
              {[
                ...
                <div key={key}>
                  ...
                  <input
                    ...
                    disabled={!isOwner || (isLocked && !isAdmin)}
                    className="w-full px-3.5 rounded-xl outline-none transition"
                    style={inputStyle(!isOwner || (isLocked && !isAdmin))}
                  />
                </div>
              ))}
              {[
                ...
                <div key={key}>
                  ...
                  <input
                    ...
                    onChange={(e) => setEditProyekForm({ ...editProyekForm, [key]: e.target.value })}
                    disabled={!isOwner || (isLocked && !isAdmin)}
                    className="w-full px-3.5 rounded-xl outline-none"
                    style={{ ...inputStyle(!isOwner || (isLocked && !isAdmin)), colorScheme: 'dark' } as React.CSSProperties}
                  />
                </div>
              ))}
              <div>
            ...
            {isOwner && (
              <button
                ...
                className="mt-5 px-6 rounded-xl text-white text-sm font-medium transition disabled:opacity-40 disabled:cursor-not-allowed"
                style={{ background: 'linear-gradient(135deg,#2563eb,#1d4ed8)', boxShadow: '0 4px 12px rgba(37,99,235,0.3)', minHeight: '44px' }}>
                {isLocked && !isAdmin ? '🔒 Terkunci' : 'Simpan Perubahan'}
              </button>
            )}
          </div>
        )}
        ...
        {activeSection === 'donor' && (
          <div>
            <div className="flex justify-between items-center mb-4">
              ...
              {canEdit && (
                <button
                  onClick={() => { setShowDonorForm(true); setEditDonor(null); setDonorForm({ nama: '', jenis: '', penanggungjawab: '', wilayah: '', alamat: '', tahunPendirian: '', lamaUsaha: '' }) }}
                ...
                <div className="p-4" style={{ background: 'rgba(255,255,255,0.02)' }}>
                  ...
                  {canEdit && (
                    <div className="flex gap-3 mt-3">
                      <button
                        onClick={() => { setEditDonor(d); setDonorForm({ nama: d.nama, jenis: d.jenis, penanggungjawab: d.penanggungjawab, wilayah: d.wilayah, alamat: d.alamat, tahunPendirian: d.tahunPendirian?.toString(), lamaUsaha: d.lamaUsaha?.toString() }); setShowDonorForm(true) }}
                    ...
                    <div className="flex items-start justify-between gap-4">
                      ...
                      <div className="flex flex-col gap-2 items-end flex-shrink-0">
                        <a href={d.fileUrl} target="_blank" rel="noopener noreferrer"
                          className="text-blue-400 text-xs hover:text-blue-300 transition" style={{ minHeight: '44px', display: 'flex', alignItems: 'center' }}>
                          Download
                        </a>
                        {isAdmin && (
                          <button
                            onClick={() => { setSelectedDokumen(d); setCatatanAdmin(d.catatanAdmin || ''); setShowApprovalModal(true) }}
        ...
        {activeSection === 'keuangan' && (
          <div>
            ...
            {transaksi.length === 0 ? (
              <p className="text-gray-600 text-sm text-center py-10">Belum ada data keuangan</p>
            ) : transaksi.map((t) => (
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-2 flex-wrap">
                      ...
                      <span className={`text-xs px-2 py-0.5 rounded-full ${
                        (t as any).statusTransaksi === 'Transfer successful' ? 'bg-emerald-500/10 text-emerald-400' :
                    ...
                    <div className="flex justify-between items-start">
                      ...
                      {canEdit && (
                        <div className="flex gap-3 ml-4">
                          <button
                            onClick={() => { setEditKegiatan(k); setKegiatanForm({ namaKegiatan: k.namaKegiatan, tanggalKegiatan: k.tanggalKegiatan.split('T')[0], fotoUrl: k.fotoUrl || '', fotoName: k.fotoName || '' }); setShowKegiatanForm(true) }}
                            className="text-blue-400 text-xs hover:text-blue-300 transition" style={{ minHeight: '44px' }}>Edit</button>
                          <button onClick={() => handleDeleteKegiatan(k.id)}
                            className="text-red-400 text-xs hover:text-red-300 transition" style={{ minHeight: '44px' }}>Hapus</button>
                        </div>
                      )}
                    </div>
            ...
            <div className="p-6">
              ...
              <div className="space-y-3">
                {[
                  ...
                  <div key={key}>
                    ...
                    <input
                      type={['tahunPendirian', 'lamaUsaha'].includes(key) ? 'number' : 'text'}
              ...
              <div className="flex gap-3 mt-6">
                <button onClick={handleSaveDonor}
                  className="flex-1 rounded-xl text-white text-sm font-medium"
                  style={{ background: 'linear-gradient(135deg,#2563eb,#1d4ed8)', minHeight: '48px' }}>Simpan</button>
                <button onClick={() => { setShowDonorForm(false); setEditDonor(null) }}
                  className="flex-1 rounded-xl text-sm text-gray-400"
                  style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.07)', minHeight: '48px' }}>Batal</button>
              </div>
            </div>
          </ModalContent>
        </ModalWrapper>
          ...
          <ModalContent wide>
            <div className="p-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-4">
                {[
                  { label: 'Nama Program *', key: 'namaProgram', placeholder: 'ketik...' },
                  { label: 'Kegiatan', key: 'kegiatan', placeholder: 'ketik...' },
                  { label: 'Staff CA *', key: 'staffCA', placeholder: 'ketik...' },
                ].map(({ label, key, placeholder }) => (
                  <div key={key}>
                    <label className="block text-xs text-gray-500 mb-1.5">{label}</label>
                    <input type="text" placeholder={placeholder}
                      value={transaksiForm[key as keyof TransaksiForm]}
                      onChange={(e) => setTransaksiForm({ ...transaksiForm, [key]: e.target.value })}
                      className="w-full px-3 rounded-lg text-white outline-none"
                      style={inputStyle()} />
                  </div>
                ))}
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-4">
                {[
                  ...
                  { label: 'Tanggal Pertanggungjawaban', key: 'tanggalPertanggungjawaban' },
                ].map(({ label, key }) => (
                  <div key={key}>
                    <label className="block text-xs text-gray-500 mb-1.5">{label}</label>
                    <input type="date"
                ...
                <div>
                  ...
                  <select value={transaksiForm.kelengkapanDokumen}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
                ...
                <div>
                  ...
                  <select value={transaksiForm.statusTransaksi}
            ...
            <div className="p-6">
              ...
              <div className="mb-4">
                ...
                <span className={`text-xs font-medium ${
                  selectedDokumen.status === 'APPROVED' ? 'text-emerald-400' :
                  selectedDokumen.status === 'REJECTED' ? 'text-red-400' : 'text-yellow-400'
                }`}>
                  {selectedDokumen.status === 'APPROVED' ? 'Disetujui' : selectedDokumen.status === 'REJECTED' ? 'Ditolak' : 'Menunggu'}
                </span>
              </div>
              <div className="mb-5">
                <label className="block text-xs text-gray-500 mb-1.5">Catatan untuk user (opsional)</label>
                <textarea value={catatanAdmin}
              ...
              <div className="flex gap-3">
                <button onClick={() => handleApproval('APPROVED')} disabled={approvalLoading}
                ...
                <button onClick={() => handleApproval('REJECTED')} disabled={approvalLoading}
                  className="flex-1 rounded-xl text-sm font-semibold transition disabled:opacity-50"
          ...
          <ModalContent>
            <div className="p-6">
              ...
              <div className="mb-5">
                ...
                <textarea value={requestEditNote}
