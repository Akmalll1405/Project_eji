...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    await prisma.$queryRawUnsafe(`
      UPDATE "Project" SET "isApproved" = false, "updatedAt" = NOW()
      WHERE id = '${id}'
    `)
    ...
    // Kirim notif ke owner dengan status khusus UNLOCKED
    ...
    if (proyekData.length > 0 && proyekData[0].userId) {
      ...
      await prisma.$queryRawUnsafe(`
        INSERT INTO "Notification" (
          ...
          '${safeNama}',
          'UNLOCKED',
          'Admin membuka kunci proyek. Kamu bisa melakukan pengeditan.',
          false, NOW()
        )
      `)
    }

    return NextResponse.json({ message: 'Project dibuka' })
  } catch (error) {
